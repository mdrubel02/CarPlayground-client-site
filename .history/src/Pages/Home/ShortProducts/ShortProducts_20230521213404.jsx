...
const ShortProducts = () => {
    ...
    const [toyotaProducts, setToyotaProducts] = useState([])
    const [fordProducts, setFordProducts] = useState([])
    const navigate = useNavigate()
    console.log(audiProducts);

    useEffect(() => {
        fetch('https://car-playground-server-site.vercel.app/products?size=6')
            .then(res => res.json())
                ...
                <div>
                    <Tabs>
                        <TabList className="">
                            ...
                            <Tab onClick={handleToyota}>Toyota</Tab>
                            <Tab onClick={handleFord}>Ford</Tab>
                        </TabList>
                        ...
                        <TabPanel>
                            <div className='sm:grid sm:grid-cols-2 lg:grid-cols-3 gap-10'>
                                {
                                    toyotaProducts.map(service => <ShortProductItems key={service._id} serviceItem={service}></ShortProductItems>)
                                }
                            </div>
                        ...
                        <TabPanel>
                            <div className='sm:grid sm:grid-cols-2 lg:grid-cols-3 gap-10'>
                                {
                                    fordProducts.map(service => <ShortProductItems key={service._id} serviceItem={service}></ShortProductItems>)
                                }
                            </div>
